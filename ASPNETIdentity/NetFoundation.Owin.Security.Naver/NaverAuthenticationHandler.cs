...
namespace NetFoundation.Owin.Security.Naver
{
    internal class NaverAuthenticationHandler : AuthenticationHandler<NaverAuthenticationOptions>
    {
        ...
        //private const string UserInfoEndpoint = "https://apis.naver.com/nidlogin/nid/getHashId_v2.xml?mode=userinfo";
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                HttpResponseMessage graphResponse = await _httpClient.SendAsync(userRequest, Request.CallCancelled);
                graphResponse.EnsureSuccessStatusCode();
                text = await graphResponse.Content.ReadAsStringAsync();

                JObject user = JObject.Parse(text);

                NaverAuthenticatedContext context = new NaverAuthenticatedContext(Context, user, accessToken, refreshToken, expires);
                ...
                if (!string.IsNullOrEmpty(context.InternalId))
                    context.Identity.AddClaim(new Claim(ClaimTypes.Sid, context.InternalId, XmlSchemaString, Options.AuthenticationType));
                ...
                if (!string.IsNullOrEmpty(context.Link))
                    context.Identity.AddClaim(new Claim(ClaimTypes.UserData, context.Link, XmlSchemaString, Options.AuthenticationType));

                if (!string.IsNullOrEmpty(context.AccessToken))
                    context.Identity.AddClaim(new Claim("urn:tokens:naver:accesstoken", context.AccessToken, XmlSchemaString, Options.AuthenticationType));
        ...
        protected override Task ApplyResponseChallengeAsync()
        {
            ...
            if (responseChallenge != null)
            {
                ...
                string authorizationEndpoint = "https://nid.naver.com/oauth2.0/authorize" + "?response_type=code" + "&client_id=" + Uri.EscapeDataString(Options.ClientId) + "&redirect_uri=" + Uri.EscapeDataString(redirectUri) + "&scope=" + Uri.EscapeDataString(scope) + "&state=" + Uri.EscapeDataString(state);
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                if (!context.IsRequestCompleted && context.RedirectUri != null)
                {
                    ...
                    if (context.Identity == null)   // add a redirect hint that sign-in failed in some way
        ...
        private static void AddQueryString(IDictionary<string, string> queryStrings, AuthenticationProperties properties, string name, string defaultValue = null)
        {
            ...
            if (!properties.Dictionary.TryGetValue(name, out str))
                str = defaultValue;
            else
